...
static NET_BMEM bool with_info;
...
/* Receiver infra */
static NET_BMEM uint32_t rx_event;
static NET_BMEM uint32_t rx_calls;
static NET_BMEM size_t info_length_in_test;
static NET_BMEM struct net_mgmt_event_callback rx_cb;
NET_DMEM static char *info_string = TEST_INFO_STRING;
static NET_BMEM char info_data[TEST_MGMT_EVENT_INFO_SIZE];
...
static int sending_event(uint32_t times, bool receiver, bool info, int line)
{
...
static int test_sending_event(uint32_t times, bool receiver, int line)
{
	return sending_event(times, receiver, false, line);
}

static int test_sending_event_info(uint32_t times, bool receiver, int line)
{
