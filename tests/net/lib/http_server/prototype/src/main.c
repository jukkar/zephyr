...
static void test_streams(void)
{
	...
	zassert_true((type == 0x1 && stream_id == 1), "Expected a HEADERS frame with stream ID 1, got %d", stream_id);
	zassert_true(offset > length, "Parsing error, buffer exceeded");
	...
	zassert_true((type == 0x0 && stream_id == 1), "Expected a DATA frame with stream ID 1, got %d", stream_id);
	zassert_true(offset > length, "Parsing error, buffer exceeded");
