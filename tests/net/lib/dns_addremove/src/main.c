...
#endif
#if defined(CONFIG_NET_IPV6)
static struct net_mgmt_event_callback mgmt_cb6;
static struct k_sem dns_added6;
static struct k_sem dns_removed6;
#endif
...
#if defined(CONFIG_NET_IPV6)
static void dns_evt_handler6(struct net_mgmt_event_callback *cb,
			      u32_t mgmt_event, struct net_if *iface)
{
	if (mgmt_event == NET_EVENT_IPV6_DNS_ADD)
	{
		k_sem_give(&dns_added6);
	}
	else if (mgmt_event == NET_EVENT_IPV6_DNS_DEL)
	{
		k_sem_give(&dns_removed6);
	}
}
#endif
...
#if defined(CONFIG_NET_IPV6)
	k_sem_init(&dns_added6, 0, 1);
	k_sem_init(&dns_removed6, 0, 1);

	net_mgmt_init_event_callback(&mgmt_cb6, dns_evt_handler6,
				     (NET_EVENT_IPV6_DNS_ADD | NET_EVENT_IPV6_DNS_DEL));
	net_mgmt_add_event_callback(&mgmt_cb6);
#endif
}
...
static void dns_do_not_add_add_callback6(void)
{
	...
	if (k_sem_take(&dns_added6, WAIT_TIME)) {
		zassert_true(true, "Received DNS added callback when should not have");
	}
#endif
}
...
static void dns_add_callback6(void)
{
	...
	if (k_sem_take(&dns_added6, WAIT_TIME)) {
		zassert_true(false, "Timeout while waiting for DNS added callback");
	}
#endif
}
...
static void dns_remove_callback6(void)
{
	...
	if (k_sem_take(&dns_removed6, WAIT_TIME)) {
		zassert_true(false, "Timeout while waiting for DNS removed callback");
	}
#endif
}
...
static void dns_remove_none_callback6(void)
{
	...
	if (k_sem_take(&dns_removed6, WAIT_TIME)) {
		zassert_true(true, "Received DNS removed callback when should not have");
	}
#endif
}
...
static void dns_add_remove_two_callback6(void)
{
	...
	ret = dns_resolve_close(dnsCtx);
	zassert_not_equal(ret, 0, "Successful result code when attempting to remove DNS server #1 again");
