...
static void check_ipv4(const struct net_cfg_interfaces *cfg)
{
	...
	ARRAY_FOR_EACH(cfg->ipv4.ipv4_addresses, i) {
		...
		zassert_true(parse_mask(cfg->ipv4.ipv4_addresses[i].value,
					...
					(struct net_sockaddr *)&saddr, &netmask_len),
	...
	ARRAY_FOR_EACH(cfg->ipv4.ipv4_multicast_addresses, i) {
		...
		ret = net_addr_pton(NET_AF_INET, cfg->ipv4.ipv4_multicast_addresses[i].value,
				    &addr);
	...
	if (cfg->ipv4.gateway[0] != '\0') {
		ret = net_addr_pton(NET_AF_INET, cfg->ipv4.gateway, &addr);
...
static void check_ipv6(const struct net_cfg_interfaces *cfg)
{
	...
	ARRAY_FOR_EACH(cfg->ipv6.ipv6_addresses, i) {
		...
		zassert_true(parse_mask(cfg->ipv6.ipv6_addresses[i].value,
					...
					(struct net_sockaddr *)&saddr, &prefix_len),
			     "Cannot parse the address \"%s\"",
	...
	ARRAY_FOR_EACH(cfg->ipv6.ipv6_multicast_addresses, i) {
		...
		zassert_true(parse_mask(cfg->ipv6.ipv6_multicast_addresses[i].value,
					...
					(struct net_sockaddr *)&saddr, NULL),
			     "Cannot parse the address \"%s\"",
