...
#if defined(CONFIG_ETH_E1000_PTP_CLOCK)
static struct device *e1000_get_ptp_clock(struct device *dev)
{
	struct e1000_dev *ctx = dev->driver_data;
...
static int ptp_clock_e1000_set(struct device *dev, struct net_ptp_time *tm)
{
...
static int ptp_clock_e1000_get(struct device *dev, struct net_ptp_time *tm)
{
	struct ptp_context *ptp_context = dev->driver_data;
...
static int ptp_clock_e1000_adjust(struct device *dev, int increment)
{
	ARG_UNUSED(dev);
...
static int ptp_clock_e1000_rate_adjust(struct device *dev, float ratio)
{
...
static int ptp_e1000_init(struct device *port)
{
	struct device *eth_dev = DEVICE_GET(eth_e1000);
	struct e1000_dev *context = eth_dev->driver_data;
...
DEVICE_AND_API_INIT(e1000_ptp_clock, PTP_CLOCK_NAME, ptp_e1000_init,
		    &ptp_e1000_context, NULL, POST_KERNEL,
