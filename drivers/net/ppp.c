...
static int save_byte(struct ppp_driver_context *ppp, u8_t byte)
{
	if (!ppp->pkt) {
		ppp->pkt = net_pkt_rx_alloc_on_iface(ppp->iface, K_NO_WAIT);
		if (!ppp->pkt) {
	...
	if (!ppp->pkt->buffer) {
		ppp->pkt->buffer = net_pkt_get_frag(ppp->pkt, K_NO_WAIT);
		if (!ppp->pkt->buffer) {
			LOG_ERR("[%p] cannot allocate 1st data buffer", ppp);
			goto out_of_mem;
		}
...
static bool check_fcs(struct ppp_driver_context *ppp)
{
	struct net_buf *buf;
	u16_t crc;
...
static void process_msg(struct ppp_driver_context *ppp)
{
	if (LOG_LEVEL >= LOG_LEVEL_DBG) {
		net_pkt_hexdump(ppp->pkt, "recv ppp");
	}

	if (IS_ENABLED(CONFIG_NET_PPP_VERIFY_FCS) && !check_fcs(ppp)) {
#if defined(CONFIG_NET_STATISTICS_PPP)
		ppp->stats.drop++;
		ppp->stats.pkts.rx++;
#endif
...
static int ppp_send(struct device *dev, struct net_pkt *pkt)
{
	...
	if (protocol > 0) {
		send_off = ppp_send_bytes(ppp, (const u8_t *)&protocol,
					  sizeof(protocol), send_off);
	}
	...
	while (buf) {
		for (i = 0; i < buf->len; i++) {
			...
			if (buf->data[i] == 0x7e || buf->data[i] == 0x7d ||
			    buf->data[i] < 0x20) {
				u8_t escaped[2];

				escaped[0] = 0x7d;
				escaped[1] = buf->data[i] ^ 0x20;

				send_off = ppp_send_bytes(ppp, escaped, 2,
							  send_off);
			} else {
	...
	fcs = sys_cpu_to_le16(fcs);
	send_off = ppp_send_bytes(ppp, (const u8_t *)&fcs, sizeof(u16_t),
				  send_off);
...
use_random_mac:
	...
	uart_pipe_register(ppp->buf, sizeof(ppp->buf), recv_cb);
}

#if defined(CONFIG_NET_STATISTICS_PPP)
static struct net_stats_ppp *get_stats(struct device *dev)
{
