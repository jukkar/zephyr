...
static bool is_valid_mux(struct device *mux)
{
	sys_snode_t *sn, *sns;
		  ...
		  void *user_data)
{
	...
	SYS_SLIST_FOR_EACH_NODE_SAFE(&uart_mux_data_devlist, sn, sns) {
		...
		if (dev_data->dev == mux_uart) {
			...
			dev_data->dlci = gsm_dlci_create(real_uart->mux,
							 mux_uart,
							 dlci_address,
							 dlci_created_cb,
							 dev_data);
...
static void uart_mux_config(struct device *dev,
			    const struct uart_mux_config *cfg)
{
	struct uart_mux_dev_data *dev_data;

	if (dev == NULL || is_valid_mux(dev) == false || cfg == NULL) {
		return;
	}

	dev_data = DEV_DATA(dev);

	memcpy(&dev_data->cfg, cfg, sizeof(dev_data->cfg));
}
