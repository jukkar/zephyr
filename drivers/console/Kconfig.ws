...
config WS_CONSOLE_LINE_BUF_SIZE
	...
	If there are a lot of short lines, then lower this value. If there
	are many long lines, then raise this value.

config WS_CONSOLE_LINE_BUF_NUMBERS
	int "WS console line buffers"
	default 4
	help
	This option can be used to modify the number of line buffers the
	driver can use. It really depends on how much output is meant to be
	sent, depending on the system load etc. You can adjust both
	WS_CONSOLE_LINE_BUF_SIZE and WS_CONSOLE_LINE_BUF_NUMBERS option to
	get the	best possible buffer settings you need.

config WS_CONSOLE_SEND_TIMEOUT
	int "WS console line send timeout"
	default 100
	help
	This option can be used to set the time limit (in milliseconds) before
	a non-empty line buffer without a linefeed character is sent.
...
config WS_CONSOLE_SEND_THRESHOLD
	...
	buffer does not contain the line feed character yet.
