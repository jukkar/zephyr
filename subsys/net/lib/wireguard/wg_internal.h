...
#define COOKIE_SECRET_MAX_AGE_MSEC (2 * SEC_PER_MIN * MSEC_PER_SEC)
#define REJECT_AFTER_TIME_MSEC     (180 * MSEC_PER_SEC)
...
struct wg_peer {
	...
	struct wg_allowed_ips allowed_source_ips[CONFIG_WIREGUARD_MAX_SRC_IPS];
	struct sockaddr cfg_endpoint; /* Configured peer IP address */
	struct sockaddr endpoint; /* Latest received IP address */
