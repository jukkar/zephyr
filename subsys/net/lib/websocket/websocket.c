...
/* If you want to see the data that is being sent, then you can enable
 * debugging and set the following variable to 1. It prints a lot of data
			   ...
			   size_t length)
{
	struct http_request *req = CONTAINER_OF(parser,
						struct http_request,
						parser);
	struct websocket_context *ctx = req->user_data;
...
int websocket_connect(int sock, const char *host, const char *url,
		      ...
		      s32_t timeout, void *user_data)
{
	...
	ret = http_client_req(sock, &req, optional_headers, response_cb,
			      &http_parser_settings, tmp_buf, tmp_buf_len,
			      timeout, ctx);
	if (ret < 0) {
		NET_DBG("[%p] Cannot connect to Websocket host %s", ctx, host);
		ret = -ECONNABORTED;
		goto out;
	}
		       ...
		       u32_t *message_type, u64_t *remaining, s32_t timeout)
{
	...
	/* Unmask the data */
	if (ctx->masked) {
		websocket_mask_payload(buf, ret, ctx->masking_value);
	}

#if HEXDUMP_RECV_PACKETS
	LOG_HEXDUMP_DBG(buf, ret, "Payload");
#endif
	...
	if (ctx->total_read >= ctx->message_len) {
		...
		if (ret > 0) {
			/* Rest of the stuff needds to go to header temp
