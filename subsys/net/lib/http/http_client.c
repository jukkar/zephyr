...
static int on_url(struct http_parser *parser, const char *at, size_t length)
{
	...
	if (req->response.http_cb && req->response.http_cb->on_url) {
		req->response.http_cb->on_url(parser, at, length);
	}
...
static int on_status(struct http_parser *parser, const char *at, size_t length)
{
	...
	len = MIN(length, sizeof(req->response.http_status) - 1);
	memcpy(req->response.http_status, at, len);
	req->response.http_status[len] = 0;

	NET_DBG("HTTP response status %d %s", parser->status_code,
		log_strdup(req->response.http_status));

	if (req->response.http_cb && req->response.http_cb->on_status) {
		req->response.http_cb->on_status(parser, at, length);
	}
			   ...
			   size_t length)
{
	...
	if (length >= len && strncasecmp(at, content_len, len) == 0) {
		req->response.cl_present = true;
	}

	print_header_field(length, at);

	if (req->response.http_cb && req->response.http_cb->on_header_field) {
		req->response.http_cb->on_header_field(parser, at, length);
	}
			   ...
			   size_t length)
{
	...
	if (req->response.cl_present) {
		if (length <= MAX_NUM_DIGITS - 1) {
			...
			req->response.content_length = num;
		}

		req->response.cl_present = false;
	}

	if (req->response.http_cb && req->response.http_cb->on_header_value) {
		req->response.http_cb->on_header_value(parser, at, length);
	}
...
static int on_body(struct http_parser *parser, const char *at, size_t length)
{
	...
	req->response.body_found = 1;
	req->response.processed += length;

	NET_DBG("Processed %zd length %zd", req->response.processed, length);

	if (req->response.http_cb && req->response.http_cb->on_body) {
		req->response.http_cb->on_body(parser, at, length);
	}

	if (!req->response.body_start &&
	    (u8_t *)at != (u8_t *)req->response.recv_buf) {
		req->response.body_start = (u8_t *)at;
	}

	if (req->response.cb) {
		if (http_should_keep_alive(parser)) {
			NET_DBG("Calling callback for partitioned %zd len data",
				req->response.data_len);

			req->response.cb(&req->response, HTTP_DATA_MORE,
					 req->user_data);
		} else {
			NET_DBG("Calling callback for %zd len data",
				req->response.data_len);

			req->response.cb(&req->response, HTTP_DATA_FINAL,
					 req->user_data);
		}

		/* Re-use the result buffer and start to fill it again */
		req->response.data_len = 0;
		req->response.body_start = NULL;
	}
...
static int on_headers_complete(struct http_parser *parser)
{
	...
	if (req->response.http_cb &&
	    req->response.http_cb->on_headers_complete) {
		req->response.http_cb->on_headers_complete(parser);
	}
...
static int on_message_begin(struct http_parser *parser)
{
	...
	if (req->response.http_cb && req->response.http_cb->on_message_begin) {
		req->response.http_cb->on_message_begin(parser);
	}
...
static int on_message_complete(struct http_parser *parser)
{
	...
	if (req->response.http_cb &&
	    req->response.http_cb->on_message_complete) {
		req->response.http_cb->on_message_complete(parser);
	}

	NET_DBG("-- HTTP %s response (complete) --",
		http_method_str(req->method));

	req->response.message_complete = 1;
...
static int on_chunk_header(struct http_parser *parser)
{
	...
	if (req->response.http_cb && req->response.http_cb->on_chunk_header) {
		req->response.http_cb->on_chunk_header(parser);
	}
...
static int on_chunk_complete(struct http_parser *parser)
{
	...
	if (req->response.http_cb &&
	    req->response.http_cb->on_chunk_complete) {
		req->response.http_cb->on_chunk_complete(parser);
	}
...
static int http_wait_data(int sock, struct http_request *req)
{
	...
	do {
		received = recv(sock, req->response.recv_buf + offset,
				req->response.recv_buf_len - offset, 0);
		if (received == 0) {
			/* Connection closed */
			...
			req->response.data_len += received;

			(void)http_parser_execute(&req->parser,
					&req->parser_settings,
					req->response.recv_buf + offset,
					received);
		}
		...
		if (offset > req->response.recv_buf_len) {
			offset = 0;
		}

		if (req->response.message_complete) {
			ret = total_received;
			break;
		}
		    ...
		    s32_t timeout, void *user_data)
{
	...
	if (sock < 0 || req == NULL || response == NULL ||
	...
	req->response.http_cb = http_cb;
	req->response.cb = response;
	req->response.recv_buf = recv_buf;
	req->response.recv_buf_len = recv_buf_len;
	...
	for (i = 0; optional_headers && optional_headers[i]; i++) {
		ret = http_send_data(sock, send_buf, send_buf_max_len,
				     &send_buf_pos, optional_headers[i], NULL);
		if (ret < 0) {
			goto out;
		}
	...
	if (req->payload_cb) {
		...
		ret = http_send_data(sock, send_buf, send_buf_max_len,
				     &send_buf_pos, HTTP_CRLF, HTTP_CRLF,
				     NULL);
		if (ret < 0) {
			goto out;
		}
	...
	http_client_init_parser(&req->parser, &req->parser_settings);
