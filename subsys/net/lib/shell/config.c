...
#if defined(CONFIG_NET_CONFIG_SETTINGS_SHELL_ACCESS)
/* Copy a shell provided value into a fixed-size configuration string field.
 * Reject values that do not fit so that we never write past the destination
 * buffer.
 */
static int copy_str_value(const struct shell *sh, char *dst, size_t dst_size,
			  const char *value)
{
	if (strlen(value) >= dst_size) {
		PR_WARNING("Value \"%s\" is too long, max %zu characters.\n",
			   value, dst_size - 1);
		return -ENOEXEC;
	}
...
#define CHECK_BASE_STR_OPTION(opt, val, cfg)				\
	if (strcmp(option, #opt) == 0) {				\
		if (copy_str_value(sh, (cfg)->opt, sizeof((cfg)->opt),	\
				   (val)) < 0) {			\
			return -ENOEXEC;				\
		}							\
		(cfg)->__##opt##_changed = true;			\
...
#define CHECK_SUB_STR_OPTION(opt, name, val, cfg)			\
	if (strcmp(option, #opt "." #name) == 0) {			\
		if (copy_str_value(sh, (cfg)->opt.name,			\
				   sizeof((cfg)->opt.name), (val)) < 0) {	\
			return -ENOEXEC;				\
		}							\
		(cfg)->opt.__##name##_changed = true;			\
		goto option_found;					\
...
#define CHECK_SUB_OPTION_ARRAY(opt, name, var, val, cfg)		\
	if (strcmp(option, #opt "." #name) == 0) {			\
		if (array_idx < 0 || array_idx >= ARRAY_SIZE((cfg)->opt.name)) { \
			...
			if (copy_str_value(sh, (cfg)->opt.name[array_idx].var, \
					   sizeof((cfg)->opt.name[array_idx].var), \
					   (val)) < 0) {		\
				return -ENOEXEC;			\
			}						\
...
static int cmd_net_config_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (strcmp(option, "flags") == 0) {
		if (copy_str_value(sh, iface_cfg->flags[0].value,
				   sizeof(iface_cfg->flags[0].value),
				   value) < 0) {
			return -ENOEXEC;
		}
