 ...
 * chapter 3.5.
 */
static uint32_t TEMP_VALID_LIFETIME = CONFIG_NET_IPV6_PE_TEMP_VALID_LIFETIME * 60;
					    ...
					    uint32_t vlifetime)
{
	...
	for (i = 0; i < NET_IF_MAX_IPV6_ADDR; i++) {
		...
		addr_age = (uint32_t)(k_uptime_get() / 1000) -
...
{
	uint8_t link_addr[NET_LINK_ADDR_MAX_LENGTH];
	...
	ifaddr->addr_create_time = (uint32_t)(k_uptime_get() / 1000);
