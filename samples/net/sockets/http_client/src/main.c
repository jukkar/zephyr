...
void main(void)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV4)) {
		(void)connect_socket(AF_INET, SERVER_ADDR4, HTTP_PORT,
				     &sock4, (struct sockaddr *)&addr4,
				     sizeof(addr4));
	}

	if (IS_ENABLED(CONFIG_NET_IPV6)) {
		(void)connect_socket(AF_INET6, SERVER_ADDR6, HTTP_PORT,
	...
	if (sock4 >= 0 && IS_ENABLED(CONFIG_NET_IPV4)) {
		...
		ret = http_client_req(sock4, &req, NULL, response_cb, NULL,
	...
	if (sock6 >= 0 && IS_ENABLED(CONFIG_NET_IPV6)) {
		...
		ret = http_client_req(sock6, &req, NULL, response_cb, NULL,
