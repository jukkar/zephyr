...
{
	char content_length[sizeof("Content-Length: ") + 5];
	...
	snprintk(content_length, sizeof(content_length),
		 "Content-Length: %zd", payload_len);
	ret = http_add_header(ctx, &pkt, content_length);
	if (ret < 0) {
